...
ui <- fluidPage(
  titlePanel("Teste Z para Proporção - Prof. Caio Azevedo"),
  fluidRow(
    column(width = 4,
           wellPanel(
             # Usando textInput para aceitar vírgula como separador decimal
             textInput("p0", "Proporção hipotética (p₀):", value = "", placeholder = "ex: 0,5"),
             textInput("phat", "Proporção amostral (p̂):", value = "", placeholder = "ex: 0,52"),
             textInput("n", "Tamanho da amostra (n):", value = "", placeholder = "ex: 100"),
  ...
  fluidRow(
    column(width = 12,
           h3("Resultados do Teste Z para Proporção", align = "center"),
           div(
             textOutput("results", 
                        container = function(...) {
...
server <- function(input, output) {
  observeEvent(input$calculate, {
    ...
    p0_input <- as.numeric(gsub(",", ".", input$p0))
    phat_input <- as.numeric(gsub(",", ".", input$phat))
    n_input <- as.numeric(gsub(",", ".", input$n))
    
    # Verificações de valores válidos
    req(!is.na(p0_input), !is.na(phat_input), !is.na(n_input))
    ...
    # Cálculo da estatística Z para proporção
    se <- sqrt(p0 * (1 - p0) / n)  
    z_stat <- (phat - p0) / se
